...
FIELDS_NUMBER_BY_SIDE = 19 # размерность игрового поля в клетках (по одной стороне, так как поле квадратное)
...
SIMULATION_PERIOD = 150  # количество лет, по истечении которых симуляция завершается
...
# КЛЕТКА (класс Field )
INIT_SOIL = 500  # количество почвы на клетке
